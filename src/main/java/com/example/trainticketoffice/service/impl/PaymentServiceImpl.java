    ...
    public Payment handleVnpayReturn(Map<String, String> vnpayParams, HttpSession session) {
        ...
        if ("00".equals(responseCode)) {
            ...
            try {
                ...
                log.error("LỖI NGHIÊM TRỌNG: Không thể cộng tiền vào ví Admin cho giao dịch {}: {}", txnRef, e.getMessage());
            }

            // Gửi email xác nhận đặt vé (bất đồng bộ, lỗi mail không làm hỏng giao dịch)
            try {
                emailService.sendBookingConfirmation(payment, ordersToUpdate);
            } catch (Exception e) {
                log.error("Không thể gửi email xác nhận cho giao dịch {}: {}", txnRef, e.getMessage());
            }
        } else {
